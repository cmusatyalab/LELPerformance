...
while (True):
    cloudlet_measurements = retrieve_timestamp(cloudlet_icmp_client, '128.2.212.53', CLOUDLET_IP, 2000, False, cloudlet_timestamp)
    xran_measurements = retrieve_timestamp(waterspout_icmp_client, '192.168.25.2', '192.168.25.4', 2000, False, xran_timestamp)
    epc_measurements = retrieve_timestamp(waterspout_icmp_client, '192.168.25.4', CLOUDLET_IP, 2000, False, epc_timestamp)
    
    # Irrelevant what we specify for UE src address (filtered out since changes each session)
    ue_measurements = retrieve_timestamp(ue_icmp_client, '192.168.25.57', CLOUDLET_IP, 2000, True, ue_timestamp)
