...
def geometryBox(fdf): # Find bounding box for a geodataframe with Points in geometry
    maxlat = fdf.geometry.y.max()
    minlat = fdf.geometry.y.min()
    maxlon = fdf.geometry.x.max()
    minlon = fdf.geometry.x.min()
    bbox = [[maxlat,maxlon],[maxlat,minlon],[minlat,maxlon],[minlat,minlon]]
    bboxdf = pd.DataFrame(bbox,columns=['LAT','LONG'])
    bboxdf['geometry'] = pt2geom(bboxdf)
    bboxgp = df2gp(bboxdf)
    return bboxgp
